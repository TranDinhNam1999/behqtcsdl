...
def format_record_development(record: dict) -> str:
    ...
    format_string = (
        "<green>{time:YYYY-MM-DDTHH:mm:ssZ}</green> <level>{level}</level>"
        + f" <yellow>{trace_id}</yellow> "  # noqa
        + "<magenta>{thread.name}-{thread.id}</magenta> <cyan>{name}</cyan>:<cyan>{function}</cyan>:<red>{line}</red> <magenta>{elapsed}</magenta> <level>{message}</level>"  # noqa
    )
